...
echo NODE_COMMIT_HASH IS "$NODE_COMMIT_HASH"
...
if [ ! -z ${NODE_FROM_SRC+x} ] || [ -n "${NODE_COMMIT_HASH:+x}" ]; then
  if [ -n "${NODE_COMMIT_HASH:+x}" ]; then
    ${NODE_FROM_SRC:=true}
...
if [ ! -z ${NODE_FROM_LOCAL+x} ]; then
  : ${NODE_FROM_SRC:=true}
  echo "Building Node from local ~/Dev/node directory!"
fi
